...
describe SessionsController do
  ...
  before do 
    ...
    Users.stub!(:authenticate).with(@login_params[:login], @login_params[:password]).and_return(@users)
  end
  def do_create
        ...
        describe "my request cookie token is #{has_request_token.to_s}," do
          describe "and ask #{want_remember_me ? 'to' : 'not to'} be remembered" do 
            before do
              ...
              @ccookies.stub!(:[]=)
              @users.stub!(:remember_me) 
              @users.stub!(:refresh_token) 
              @users.stub!(:forget_me)
              @users.stub!(:remember_token).and_return(token_value) 
              @users.stub!(:remember_token_expires_at).and_return(token_expiry)
              @users.stub!(:remember_token?).and_return(has_request_token == :valid)
              if want_remember_me
            ...
            if (has_request_token == :valid)
              it 'does not make new token'   do @users.should_not_receive(:remember_me);   do_create end
              it 'does refresh token'        do @users.should_receive(:refresh_token);     do_create end 
              it "sets an auth cookie"       do do_create;  end
            else
              if want_remember_me
                it 'makes a new token'       do @users.should_receive(:remember_me);       do_create end 
                it "does not refresh token"  do @users.should_not_receive(:refresh_token); do_create end
                it "sets an auth cookie"       do do_create;  end
              else 
                it 'does not make new token' do @users.should_not_receive(:remember_me);   do_create end
                it 'does not refresh token'  do @users.should_not_receive(:refresh_token); do_create end 
                it 'kills user token'        do @users.should_receive(:forget_me);         do_create end 
              end
            end
          end # inner describe
        end
  ...
  describe "on failed login" do
    before do
      Users.should_receive(:authenticate).with(anything(), anything()).and_return(nil)
      login_as :quentin
    end
